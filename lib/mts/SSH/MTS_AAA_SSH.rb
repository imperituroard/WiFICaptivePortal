...
class MTSssh
  ...
  def carcommand_autorize(ipaddr,subscriber_number,onetimepassword)
    ...
    password_car = conf.carSSH_password
    Net::SSH.start(host_car, username_car, :password => password_car) do |ssh|
       command = %[/opt/CSCOar/bin/coa_lin -n 172.24.207.41 -p 1700 -k wificisco  -1 26,9,250,"S#{ipaddr}:vrf-id=global" -2 26,9,1,"subscriber:command=account-logon" -3 1,"#{subscriber_number}" -4 26,9,1,"subsciber:password=#{onetimepassword}"]
      # p com =command.gsub!('\\','')

...
#result = client.query("SELECT * FROM table", :cast => false)
  ...
  def addText(id, company_id, text)
    u=Parameters.new
    ...
    client = Mysql2::Client.new(:host => host_db, :username => username_db, :password => password_db)
    client.query("use mtsPortalWiFI", :cast => false)
    client.query("INSERT INTO texts_Portal_wifi  (TextID, organizationID, textPage) VALUES(#{id}, #{company_id}, '#{text}' )")
  end
  ...
  def checkifSMSwasSend(ipaddress)
    ...
    client = Mysql2::Client.new(:host => host_db1, :username => username_db1, :password => password_db1)
    client.query("use mtsPortalWiFI", :cast => false)
    dffd1= client.query("SELECT verificationcode FROM users_wifi WHERE ipaddress=INET_ATON('#{ipaddress}') and time <=DATE_SUB(CURRENT_DATE, INTERVAL 5 MINUTE) ORDER BY time DESC LIMIT 1",:as => :array).each do |row|
      p row[0]
      if row[0].length ==4
            return '3'
        else return nil
